...
def REngage(id,data,G):
    ...
    if('tweet_retweets' in data):
        ...
        for i in range(len(data['tweet_retweets'])):
            ...
            creationTime = data['tweet_retweets'][i]['created_at']
            G.add_node(str(rid), id=3, user=verified, usrType=userType, CreatedAt=creationTime)
        ...
        for i in range(len(ids)):
            ...
            if (rdata[index]!= None):
                for j in range(len(rdata[index])):
                    ...
                    creationTime=rdata[index][j]['created_at']
                    G.add_node(str(rid),id=3,user=verified,usrType=userType,CreatedAt=creationTime)
...
f=open("C:\\Users\\mansour\Desktop\\politifact_result_fake.txt","w+")
...
w = csv.writer(open("C:\\Users\\mansour\Desktop\\output.csv", "w"))

for key, value in sorted(news_sources.items(), key = itemgetter(1), reverse = True):
